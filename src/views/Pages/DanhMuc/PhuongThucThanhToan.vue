    ...
    <div class="wrapper m-2">
         <div class="col-md-12 form-row pt-2">
            <div class="col-md-8 d-flex">
                    ...
                    <Input placeholder="Nhập phương thức thanh toán"  v-model="search" />
            ...
            <el-card style="margin-top: 10px;">
                     ...
                     <div style="padding: 5px 10px;" class="form-row">
                        <div class="col-md-6">
                            <base-button type="primary" outline native-type="submit" @click="handleAdd">
                                ...
                                <span class="btn-inner--text">&nbsp;Thêm phương thức</span>
                            </base-button>
                        </div>
                     </div>
                </template>
                ...
                <div class="col-md-12 pt-2 tbl" >
                        <el-table :data="tableData.content" fit header-row-class-name="thead-light" height="100%" :border="true" stripe>
                            <el-table-column label="Mã phương thức" prop="nMaPhuongThuc" header-align="center">

                            </el-table-column>
                            <el-table-column label="Tên phương thức thanh toán" prop="cTenPhuongThuc" width="500" header-align="center">
        ...
        <modal :show.sync="dialogForm">
            <h6 slot="header" class="modal-title" v-if="!operation">Thêm phương thức thanh toán</h6>
            <h6 slot="header" class="modal-title" v-if = "operation">Cập nhật phương thức thanh toán</h6>
                <el-form :model="dataForm" label-width="150px" ref="dataForm">
                    <el-form-item label="Tên phương thức" prop="cTenPhuongThuc" :rules="[{required: true, message:'Bạn chưa nhập phương thức thanh toán'}]">
                        <el-input size="mini" v-model="dataForm.cTenPhuongThuc"></el-input>
                    </el-form-item>
                </el-form>
                  ...
                  handleAdd(){
                    ...
                    findPage(pageNum,pageSize){
                        ...
                        this.pageRequest.columnFilters = { 
                            cTenPhuongThuc: {value: this.search}
                        }
                        this.$api.paymentmethod.findPage(this.pageRequest).then((res) => {
                             if(res.code == 200){
                    ...
                    save(){
                        this.$refs.dataForm.validate((valid) =>{
                            if(valid){
                                ...
                                this.$api.paymentmethod.insert(data) 
                                .then((res) => {
                                    if(res.code == 200){    
                    ...
                    update(){
                        this.$refs.dataForm.validate((valid) =>{
                            if(valid){
                                let data = Object.assign({}, this.dataForm)
                                this.$api.paymentmethod.update(data).then((res) => {
                    ...
                    deleteBatch(index,item){
                        this.$confirm("Bạn có muốn xóa phương thức thanh toán này không?",'Xác nhận',{
                            confirmButtonText: 'Xác nhận',
                            cancelButtonText: 'Hủy',
                            type: 'warning'
                            }).then(()=>{
                            let params = item.nMaPhuongThuc  
                            this.$api.paymentmethod.deleteBatch(params).then((res) => {
                            if (res.code == 200) {
                                this.$message({message: res.msg, type:'success'})
                                this.findPage()
                            } else {
