    ...
    <div class="wrapper m-2">
        <div class="col-md-12 form-row pt-2">
            <div class="col-md-6 d-flex">
                    ...
                    <Input placeholder="Nhập loại khách hàng" v-model="search" />
            ...
            <el-card style="margin-top: 10px">
                     ...
                     <div style="padding: 5px 10px;" class="form-row">
                        <div class="col-md-6">
                            <base-button type="primary" outline native-type="submit" @click="handleAdd">
                                ...
                                <span class="btn-inner--text">&nbsp;Thêm loại khách hàng</span>
                            </base-button>
                        </div>
                     </div>
                </template>
                ...
                <div class="col-md-12 pt-2 tbl" >
                        <el-table :data="tableData.content" fit header-row-class-name="thead-light" height="100%" :border="true" stripe>
                            <el-table-column label="Mã loại khách hàng" prop="nMaLoaiKhachHang" header-align="center">

                            </el-table-column>
                            <el-table-column label="Tên loại khách hàng" prop="cTenLoaiKhachHang"  header-align="center">
        ...
        <modal :show.sync="dialogForm">
            <h6 slot="header" class="modal-title" v-if="!operation">Thêm loại khách hàng</h6>
            <h6 slot="header" class="modal-title" v-if = "operation">Cập nhật thông tin loại khách hàng</h6>
                <el-form :model="dataForm" label-width="160px" ref="dataForm">
                    <el-form-item label="Tên loại khách hàng" prop="cTenLoaiKhachHang" :rules="[{required: true, message:'Bạn chưa nhập tên loại khách hàng'}]">
                        <el-input size="mini" v-model="dataForm.cTenLoaiKhachHang"></el-input>
                    </el-form-item>
                </el-form>
            ...
            methods: {
                    ...
                    findPage(pageNum,pageSize){
                        ...
                        this.pageRequest.columnFilters = { 
                            cTenLoaiKhachHang: {value: this.search}
                        }
                        this.$api.customertype.findPage(this.pageRequest).then((res) => {
                             if(res.code == 200){
                     ...
                     save(){
                        this.$refs.dataForm.validate((valid) =>{
                            if(valid){
                                ...
                                this.$api.customertype.insert(data) 
                                .then((res) => {
                                    if(res.code == 200){    
                    ...
                    update(){
                        this.$refs.dataForm.validate((valid) =>{
                            if(valid){
                                let data = Object.assign({}, this.dataForm)
                                this.$api.customertype.update(data).then((res) => {
                    ...
                    deleteBatch(index,item){
                        this.$confirm("Bạn có muốn xóa loại khách hàng này không?",'Xác nhận',{
                            confirmButtonText: 'Xác nhận',
                            cancelButtonText: 'Hủy',
                            type: 'warning'
                            }).then(()=>{
                            let params = item.nMaLoaiKhachHang  
                            this.$api.customertype.deleteBatch(params).then((res) => {
                            if (res.code == 200) {
                                this.$message({message:res.msg, type:'success'})
                                this.findPage()
                            } else {
